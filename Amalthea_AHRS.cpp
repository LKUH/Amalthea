...
Amalthea_AHRS::Amalthea_AHRS(UpdateMethod _updateMethod)
{
	acc = initializeMarg_2();
	
	updateMethod = _updateMethod;
}

ADXL345* Amalthea_AHRS::initializeMarg_2()
{
	acc->powerOn();
	
}

void Amalthea_AHRS::Amalthea_AHRS_2(UpdateMethod _updateMethod)
{	
...
void Amalthea_AHRS::initializeMarg()
{
	...
	// HMC5883L(); //reset magnetometer
    compass->SetScale(1.3); // Set the scale to +/- 1.3 Ga of the compass
    compass->SetMeasurementMode(Measurement_Continuous);

	// marg = new Amalthea(MODE_I2C, Amalthea_G, Amalthea_XM);
...
void Amalthea_AHRS::readMarg()
{
	...
	MagnetometerRaw raw_m = compass->ReadRawAxis();
	mx = raw_m.XAxis;
	my = raw_m.YAxis;
	mz = raw_m.ZAxis;	
...
void Amalthea_AHRS::updateQuaternions()
{
	if (updateMethod == QAEF)
		QAEFQuaternionUpdate(ax, ay, az, gx * PI / 180.0f, gy * PI / 180.0f, gz * PI / 180.0f, mx, my, mz);
	else
		mahonyQuaternionUpdate(ax, ay, az, gx * PI / 180.0f, gy * PI / 180.0f, gz * PI / 180.0f, mx, my, mz);
}

void Amalthea_AHRS::updateEulerAngles()
{
...
// Implementation of Sebastian QAEF's "...efficient orientation filter for... inertial/magnetic sensor arrays"
// (see http://www.x-io.co.uk/category/open-source/ for examples and more details)
// which fuses acceleration, rotation rate, and magnetic moments to produce a quaternion-based estimate of absolute
// device orientation -- which can be converted to yaw, pitch, and roll. Useful for stabilizing quadcopters, etc.
// The performance of the orientation filter is at least as good as conventional Kalman-based filtering algorithms
// but is much less computationally intensive---it can be performed on a 3.3 V Pro Mini operating at 8 MHz!
void Amalthea_AHRS::QAEFQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
{
...
// Similar to QAEF scheme but uses proportional and integral filtering on the error between estimated reference vectors and
// measured ones. 
void Amalthea_AHRS::mahonyQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
{
  float q1 = q[0], q2 = q[1], q3 = q[2], q4 = q[3];   // short name local variable for readability
  float norm;
  float hx, hy, bx, bz;
